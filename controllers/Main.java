...
public class Main {
  ...
  /**
   * Constructor sets up the Spacebook system by creating a list of users having a selection of
   * users make friends having some users engage in conversation by sending each other messages
   */
  public Main() {
    createUsers();
    createFriendships();
   ...
   *********************** The main method**********************************************
   * 
   * Simulates the Play LeaderBoard and Home controller functionality Uses Sedgewick & Wayne library
   * to input choice in console Input args via console
   * 
   * @param args program arguments integers in range [0 4] as follows:
   * 
   *        Spacebook: Home page for <user> 0 runs Home.index (equivalent to BY DATE button press in
   *        Play app) 1 runs Home.byUser (equivalent to BY USER button press) 2 runs
   *        Home.byConversation (equivalent to BY CONVERSATION button press)
   * 
   *        Social Leaderboard 3 runs LeaderBoard.index (equivalent to MOST SOCIAL button press) 4
   *        runs LeaderBoard.talkative (equivalent to MOST TALKATIVE button press) 5 runs
   *        LeaderBoard.leastTalkative (new feature)
   * 
   *************************************************************************************/
